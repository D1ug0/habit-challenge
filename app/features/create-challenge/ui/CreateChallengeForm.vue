...
async function submit(): Promise<void> {
  ...
  if (!result.success) {
    fieldErrors.value = Object.fromEntries(result.error.issues.map(issue => [issue.path.join('.'), issue.message]))
    return
  }
  ...
  <section class="create-page">
    ...
    <form class="form-card card" @submit.prevent="submit">
      <div class="field">
        ...
        <input id="title" v-model="form.title" class="input" maxlength="80" placeholder="Например, читать 20 минут" autocomplete="off">
      ...
      <div class="field">
        <label class="field-label" for="description">Зачем тебе это? <span>необязательно</span></label>
        <textarea id="description" v-model="form.description" class="input" maxlength="500" placeholder="Короткое напоминание для себя" />
      ...
      <button class="button-primary submit" type="submit" :disabled="submitting || session.status !== 'ready'">
...
<style scoped>
.create-page { display: grid; gap: 24px; }
.back-link { color: var(--muted); text-decoration: none; font-size: 0.82rem; font-weight: 750; }
header { max-width: 570px; }
h1 { margin: 8px 0 16px; font-family: Georgia, serif; font-size: clamp(2.5rem, 12vw, 4.7rem); line-height: 0.94; letter-spacing: -0.055em; font-weight: 500; }
h1 em { color: var(--accent); font-weight: inherit; }
.form-card { padding: 22px; display: grid; gap: 23px; }
.field-label span { color: var(--muted); font-weight: 500; }
.field-error { color: var(--danger); font-size: 0.76rem; }
.emoji-picker { display: flex; gap: 7px; overflow-x: auto; padding: 3px; }
.emoji-picker button { width: 48px; height: 48px; flex: 0 0 48px; border-radius: 14px; border: 1px solid var(--line); background: var(--surface); font-size: 1.35rem; cursor: pointer; }
.emoji-picker button.selected { border-color: var(--accent); background: var(--accent-soft); transform: translateY(-2px); }
fieldset { border: 0; padding: 0; margin: 0; display: grid; gap: 8px; }
.segmented { display: grid; grid-template-columns: 1fr 1fr; gap: 9px; }
.segmented label { border: 1px solid var(--line); border-radius: 15px; padding: 13px 14px; cursor: pointer; display: grid; gap: 2px; }
.segmented label.selected { border-color: var(--accent); background: var(--accent-soft); }
.segmented input { position: absolute; opacity: 0; }
.segmented small { color: var(--muted); }
.two-columns { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.submit { width: 100%; }
@media (max-width: 480px) { .two-columns { grid-template-columns: 1fr; } .form-card { padding: 18px; } }
</style>
