...
Открытые и закрытые группы, приглашения, управление участниками и совместный прогресс.
...
Приложение считает серию, прогресс и показывает активность за последние 30 дней.
...
GIF выше собран из кадров локального Playwright-сценария. Для обновления используйте
`scripts/capture-demo.mjs` и `scripts/assemble-demo.py`.
...
5. создаёт отзывную серверную сессию в PostgreSQL.
...
В профиле можно изменить часовой пояс, настроить напоминания, завершить отдельные сессии и удалить Telegram-аккаунт.
...
Календарный день check-in определяется сервером в часовом поясе пользователя, который браузер
передаёт при авторизации. Период группового челленджа фиксируется в часовом поясе его создателя,
а streak каждого участника считается по его локальной календарной дате.
...
Для закрытой группы к параметру добавляется секрет приглашения: `challenge_<id>_<token>`.
...
Создатель может досрочно завершить челлендж: история и лидерборд сохраняются, новые отметки и вступления блокируются. Автоматическое завершение наступает после последнего календарного дня в часовом поясе челленджа. Создатель может удалить челлендж целиком; это удаляет также участников и их отметки.

Название, описание и emoji можно редактировать. Закрытая группа требует секрет из ссылки-приглашения;
создатель может исключить участника, запретить повторный вход и затем снять запрет.
...
После обновления кода повторно запустите `pnpm db:migrate`.
...
Для напоминаний у пользователя должен быть начат личный диалог с ботом. В профиле он включает
напоминание и выбирает местный час. В production отдельный scheduler вызывает защищённый endpoint
каждые пять минут; отправка каждого дня дедуплицируется в PostgreSQL.

## Production

Готовая конфигурация для VPS с Docker Compose, Caddy (HTTPS), scheduler и резервными копиями
описана в [инструкции по развёртыванию](ops/DEPLOYMENT.md). Понадобятся домен, PostgreSQL,
Telegram Bot Token, секреты приложения и S3 bucket. До заполнения этих реквизитов публичное
...
Резервная копия PostgreSQL через установленный `pg_dump` (для локальной базы команда автоматически
использует контейнер из Docker Compose, если клиент PostgreSQL не установлен):

```bash
pnpm db:backup
```

Каталог можно изменить переменной `BACKUP_DIR`, а путь к нестандартно установленному `pg_dump` —
переменной `PG_DUMP_PATH`. В production резервное копирование следует запускать по расписанию и
хранить копии вне сервера приложения. Команда загружает локальные настройки сначала из
`.env.example`, затем переопределяет их значениями из `.env` и окружения процесса.
...
- более подробной аналитикой и календарным видом;
- дополнительными режимами челленджей;
- несколькими ролями модераторов;
- реакциями участников;
- push-сценариями через Telegram Bot API;
- дополнительными отчётами для организаторов групп.
