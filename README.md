...
Календарный день check-in определяется сервером в часовом поясе челленджа. Этот пояс фиксируется
при создании, поэтому все участники отмечают один и тот же день челленджа, а смена часового пояса
профиля не даёт повторно отметить выполнение. Личный часовой пояс используется для напоминаний и
отображения активности в общей статистике. Миграция `0007` переводит ранее сохранённые отметки
на дни челленджа; если две старые отметки попали в один такой день, остаётся первая из них.
...
Создатель может досрочно завершить челлендж: история и лидерборд сохраняются, новые отметки, вступления и исключения участников блокируются. Автоматическое завершение наступает после последнего календарного дня в часовом поясе челленджа. Создатель может удалить челлендж целиком; это удаляет также участников и их отметки.
